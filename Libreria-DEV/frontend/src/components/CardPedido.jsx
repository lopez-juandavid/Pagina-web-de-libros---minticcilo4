...
const CardPedido = ({ info = {}, boton = true, callUpdate }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://backend-eraseunavez.herokuapp.com/usuarios/${info.cliente}`)
      .then((ans) => {
  ...
  const handleConfirmar = () => {
    axios
      .put(`https://backend-eraseunavez.herokuapp.com/pendientes/${info._id}`)
      .then(() => {
        callUpdate();
      })
      .catch((err) => {
  ...
  return (
    <div
      className={`card my-3 border-2 border-warning ${
        boton ? "sombra2" : "sombra"
      }`}
      style={{ width: "18rem" }}
    >
